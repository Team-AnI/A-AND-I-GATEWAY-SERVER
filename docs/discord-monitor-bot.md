...
운영자가 외워야 하는 top-level command는 `/ops` 하나다. 코드에는 기존 command를 임시 호환 alias로 처리하는 경로가 남아 있지만, 기본 Discord registration은 `/ops`만 등록한다.
...
`/ops storage view:usage`와 `/ops storage view:retention`은 CloudWatch `DescribeLogGroups` 기반 조회 전용 명령이다. host Docker disk를 직접 조회하거나 삭제하지 않고, Docker socket도 마운트하지 않는다. retention이 없는 log group은 `INFINITE`로 표시한다. 임시 legacy alias를 등록한 경우 `/disk`, `/retention`도 같은 조회 로직을 사용한다.
