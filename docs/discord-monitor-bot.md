...
- 새 채널 env는 추가하지 않는다.
- `/ops alert action:channel channel:#채널`로 저장한 state 값을 우선 Assignment Ops 채널로 사용한다.
- state에 alert channel이 없으면 `DISCORD_ALERT_CHANNEL_ID`, 그다음 `DISCORD_DASHBOARD_CHANNEL_ID`를 사용한다.
- Assignment Ops는 실시간 webhook이 아니라 poll 기반이다. 기본 poll 주기는 alert poll interval을 재사용하며 보통 3분이다.
- 첫 성공 poll은 baseline만 저장하고 과거 assignment 이벤트를 대량 발송하지 않는다. 이후 변경분부터 dashboard/event feed에 반영한다.
