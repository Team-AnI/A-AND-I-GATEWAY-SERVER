...
과제 dashboard의 최근 이벤트는 eventType/course/summary/reason 기준으로 묶어서 표시합니다. WEB Admin snapshot diagnosis 이벤트는 traceId가 없을 수 있으므로 assignmentId, issueKey, evidenceHash, `/ops assignment course:<course> id:<assignmentId> view:events` 중심으로 drilldown합니다. Report EVENT audit 이벤트는 실제 EVENT 로그에 traceId/requestId가 있을 때만 trace drilldown을 표시합니다.
...
| `/ops assignment-check` | `/ops assignment action:check` |
| `/ops assignment-events` | `/ops assignment view:events` |
| `/ops assignment-ack` | `/ops assignment action:ack` |
| `/ops assignment-unack` | `/ops assignment action:unack` |
| `/ops submissions` | `/ops assignment action:submissions` |
