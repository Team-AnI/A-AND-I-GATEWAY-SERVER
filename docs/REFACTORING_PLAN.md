...
| 5 | Monitor Bot 대형 모듈 분리 | 5A, 5B, 5C-a, 5C-b1 완료; 5C-b2 다음 | 동작 변경 없는 파일 분리 후 Go 전체 테스트와 race 통과 |
...
### 5C-b2: dashboard command family 분리 — 다음

1. dashboard 조회, watch, unwatch와 status handler를 `interactions_dashboard.go`로 이동한다.
