...
| 4 | route와 보안 정책 source of truth 단일화 | 4A, 4B-1, 4B-2a 완료; 4B-2b 다음 | 실제 route 선택과 route별 선언 method 도달성 검사 후 typed contract로 단계적 전환 |
...
#### 4B-2b: Auth endpoint policy/access catalog — 다음

1. ordered request matcher를 표현하는 `EndpointPolicyContract`와 `AccessContract`를 route catalog와 분리해 추가한다.
2. ROUTED endpoint와 LOCAL endpoint를 구분하고 `GatewayRequestPolicyFilter`의 deny-before-allow 평가와 `SecurityConfig`의 first-match access를 선언 순서까지 고정한다.
3. 하나의 route에 여러 endpoint rule이 대응하는 경우, exact/broad path 중첩, Method 미지정 route의 OPTIONS 동작을 witness test로 보존한다.
4. `/v2/auth/me/password`의 authenticated fallback과 역할 제한 비대칭은 동작 변경 없이 review-required 항목으로 기록한다.
5. dry-run catalog와 현재 `GatewayRequestPolicyFilter`·`SecurityConfig`의 동등성을 먼저 검증한다.
6. 동등성이 확보된 뒤에만 런타임 소비자를 서비스 단위로 전환한다.
7. 실제 handler나 route가 없는 legacy allow rule은 별도 테스트와 사용처 확인 후 제거한다.
