...
| 4 | route와 보안 정책 source of truth 단일화 | 4A, 4B-1, 4B-2a, 4B-2b, 4B-2c, 4B-2d-a 완료; 4B-2d-b 다음 | 실제 route 선택과 route별 선언 method 도달성 검사 후 typed contract로 단계적 전환 |
...
#### 4B-2d-b: global access catalog runtime 소비 — 다음
...
4. 접근 등급, rule 순서, winning rule ID가 하나라도 달라지면 runtime 전환을 중단한다.

#### 4B-2d-c: Auth access projection 중복 정리 — 계획
