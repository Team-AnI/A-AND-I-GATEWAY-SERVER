...
| 5 | Monitor Bot 대형 모듈 분리 | 5A, 5B, 5C-a 완료; 5C-b 다음 | 동작 변경 없는 파일 분리 후 Go 전체 테스트와 race 통과 |
...
### 5C-b: interaction command 분리 — 다음

1. interaction handler는 활성 command family 단위로 한 파일씩 이동한다.
2. command schema, option parsing, 응답 문자열과 timeout은 변경하지 않는다.
