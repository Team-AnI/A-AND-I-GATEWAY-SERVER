...
| 5 | Monitor Bot 대형 모듈 분리 | 5A 완료; 5B 다음 | 동작 변경 없는 파일 분리 후 Go 전체 테스트와 race 통과 |
...
### 5B: assignment formatting 분리 — 다음

1. assignment 응답 formatter와 전용 타입을 `assignment_format.go`로 이동한다.
2. 공용 sanitize, truncate, row summary helper는 기존 파일에 유지한다.
3. 출력 문자열과 exported signature는 변경하지 않는다.

### 5C: assignment lifecycle과 interaction command 분리 — 계획
