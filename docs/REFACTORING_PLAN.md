...
| 4 | route와 보안 정책 source of truth 단일화 | 4A, 4B-1, 4B-2a, 4B-2b, 4B-2c 완료; 4B-2d 다음 | 실제 route 선택과 route별 선언 method 도달성 검사 후 typed contract로 단계적 전환 |
...
- ordered matcher 13개와 fallback, flattened pattern 27개 fingerprint 일치
- Auth 73개 ordered access projection과 접근 등급 분포 일치
- `./gradlew test --tests com.aandi.gateway.security.AuthAccessPolicyCatalogTests --tests com.aandi.gateway.security.AuthAccessPolicyCatalogIntegrationTests --no-daemon`
- `./gradlew test bootJar --rerun-tasks --no-daemon`
- `git diff --check`

#### 4B-2d: global access catalog runtime 전환 — 다음

1. Auth 전용 projection을 곧바로 runtime에 연결하지 않는다. `/v2/*/v3/api-docs`, `/v1/admin/**`, `/v2/admin/**`가 여러 서비스와 LOCAL endpoint에 공유되기 때문이다.
2. authenticated chain의 path matcher 37개와 최종 any-exchange 1개를 전역 ordered catalog로 확장해 현재 선언 순서와 winning rule을 먼저 dry-run 검증한다.
3. `gateway.auth.enabled=false`의 별도 permit-all chain을 독립 계약으로 유지한다.
4. 전역 catalog와 현재 `SecurityConfig`의 동등성이 확보된 뒤에만 runtime 소비자로 전환한다.
5. 실제 handler나 route가 없는 legacy rule은 별도 사용처 확인과 테스트 후 제거한다.
