...
| 4 | route와 보안 정책 source of truth 단일화 | 4A, 4B-1, 4B-2a, 4B-2b 완료; 4B-2c 다음 | 실제 route 선택과 route별 선언 method 도달성 검사 후 typed contract로 단계적 전환 |
...
#### 4B-2c: Auth access catalog dry-run — 다음

1. ordered `AccessContract`를 route catalog와 endpoint policy catalog에서 분리해 dry-run catalog로 추가한다.
2. `SecurityConfig`의 permit-all, authenticated, role 제한과 first-match 선언 순서를 현재 동작 그대로 기록한다.
3. ROUTED/LOCAL endpoint 구분, exact/broad path 중첩, Method 미지정 route의 OPTIONS 동작을 witness test로 보존한다.
4. `/v2/auth/me/password`의 authenticated fallback과 역할 제한 비대칭을 동작 변경 없이 review-required 항목으로 기록한다.
5. dry-run catalog와 현재 `SecurityConfig`의 동등성이 확보된 뒤에만 4B-2d에서 runtime 소비자로 전환한다.
6. 실제 handler나 route가 없는 legacy rule은 별도 사용처 확인과 테스트 후 제거한다.
