...
- `/ops logs service:<all|gateway|auth|report|blog> mode:<recent|errors|slow|security> level:<INFO|WARN|ERROR> limit:<5|10|20>`
...
- `CRITICAL` alert만 `/ops alert action:role`로 저장한 role을 mention합니다. 없으면 `DISCORD_ALLOWED_ROLE_IDS`의 첫 번째 role을 fallback으로 사용하고, role 값이 없으면 mention 없이 전송합니다. `HIGH` alert는 alert channel로 전송하지만 role mention은 하지 않습니다.
