...
Primary 운영 command는 `/ops`이며, 기본 help에 노출하는 command family는 5개입니다.

- `/ops dashboard [service:<gateway|auth|report|blog>] [action:<view|watch|unwatch|status>] since:<15m|30m|1h> channel:#ops interval:<1m|3m|5m|10m|15m>`
- `/ops logs [action:<view|watch|unwatch|watches>] [service:<all|gateway|auth|report|blog>] mode:<recent|errors|critical|slow|security|events|trace> level:<INFO|WARN|ERROR> query:<assignmentId|traceId|actorId|eventType|errorCode> limit:<5|10|20>`
- `/ops alert action:<channel|role|role-clear|on|off|status|test> target:<all|general|critical> channel:#ops-alerts role:@운영팀`
- `/ops assignment [scope:<course|all>] [course:<courseSlug>] [id:<assignmentId>] [action:<list|check|submissions|ack|unack>] [view:<summary|diagnosis|raw|events>]`
- `/ops help topic:<overview|dashboard|logs|alerts|assignments> command:<dashboard|logs|alert|assignment> query:<text>`

기존에 별도 subcommand로 나뉘던 기능은 위 5개 family의 option으로 흡수합니다.

- `/ops dashboard service:report`는 기존 service 조회 역할입니다.
- `/ops dashboard action:watch|unwatch|status`는 기존 dashboard watch 역할입니다.
- `/ops logs mode:trace query:<traceId>`는 기존 trace drilldown 역할입니다.
- `/ops logs action:watch|unwatch|watches`는 기존 log feed watch 역할입니다.
- `/ops assignment action:list|check|submissions|ack|unack`와 `view:events`는 기존 과제 보조 명령 역할입니다.

Service Ops 자동화 설정:

- `/ops dashboard action:watch channel:#ops interval:5m`
- `/ops dashboard action:watch service:blog channel:#blog-ops interval:5m`
...
- `/ops logs action:watch service:blog mode:<errors|slow|recent|security> channel:#blog-logs interval:5m since:30m limit:10`
- `/ops logs action:unwatch service:blog mode:errors`
...
CloudWatch Logs는 `/ops logs`, admin API 실패 시 fallback 확인 용도로만 사용합니다. trace drilldown은 `/ops logs mode:trace query:<traceId>`로 수행합니다. fallback 응답은 `fallback result, not authoritative`로 표시되며, 과제 존재/공개/제출 상태의 authoritative source는 WEB-SERVER admin API입니다.
...
- dashboard는 `/ops dashboard action:watch ... channel:#채널`로 저장한 채널/messageId를 우선 사용하고, 없으면 기존 `DISCORD_DASHBOARD_CHANNEL_ID`를 사용합니다. `channel` 옵션을 생략하면 명령어를 실행한 현재 채널에 고정합니다.
...
- state는 `/var/lib/monitor-bot/state.json`에 저장하며 dashboard watch, alert 설정, log feed fingerprint를 보관합니다. 저장은 atomic rename 방식으로 수행하고, 깨진 state는 `.corrupt.*` 파일로 보존한 뒤 빈 state로 graceful fallback합니다.
- `/ops logs action:watch`는 최초 등록 시 기존 결과를 baseline fingerprint로 저장하고, 이후 새 항목부터 전송합니다.
...
상세 분석은 자동 alert에 raw log를 싣지 않고 slash command로 진행합니다. trace drilldown은 `/ops logs mode:trace query:<traceId>`로 수행하며, `/ops logs` 또는 log feed 결과에 traceId가 있을 때만 Next command로 노출됩니다.
...
assignment 관련 `/ops` option은 관리자 페이지를 대체하는 상태 확인 UI가 아닙니다. feed에서 이상 항목을 봤을 때 상세 확인하거나, admin API 장애 시 fallback 확인하는 용도입니다.
