...
- `/ops logs-watch service:report mode:<errors|slow|recent> interval:5m since:30m limit:10`
...
- alert는 `/ops alert action:channel`로 저장한 채널을 우선 사용하고, 없으면 기존 `DISCORD_ALERT_CHANNEL_ID`로 전송합니다.
- dashboard는 `/ops watch`로 저장한 채널/messageId를 우선 사용하고, 없으면 기존 `DISCORD_DASHBOARD_CHANNEL_ID`를 사용합니다.
